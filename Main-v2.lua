...
-- دالة لتحويل asset ID إلى custom asset
local function convertToCustomAsset(assetId)
    ...
    if type(assetId) == "string" and assetId:match("^rbxassetid://(%d+)$") then
        ...
        local customPath = "infiniteyield/assets/" .. id .. ".png"
        
        if isfile and isfile(customPath) then
            return "rbxasset://" .. customPath
        end
    end
...
function IconModule.AddIcons(packName, iconsData)
    ...
    for iconName, iconValue in pairs(iconsData) do
        if type(iconValue) == "number" or (type(iconValue) == "string" and iconValue:match("^rbxassetid://")) then
            ...
            -- تحويل إلى custom asset إذا كان الملف موجود
            imageId = convertToCustomAsset(imageId)
...
function IconModule.Icon(Icon, Type)  
    ...
    if iconSet and iconSet.Icons[targetName] then  
        local iconData = iconSet.Icons[targetName]
        local imageId = iconData.Image
